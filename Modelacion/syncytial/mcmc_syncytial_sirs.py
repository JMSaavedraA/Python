...
class direct_problem:
    ...
    def solve(self,p):
        ...
        Use the endpoint as initial condition to simulate the system in the data
        where there is data.
        ...
        """
        y0 = np.array([1.0-p[2]-p[3],p[2],p[3],p[4],p[5],sirs.data[0]])
...
if __name__=="__main__":
    ...
    def energy(p):
        ...
        """
        log_prior = 0.0 #initialize the logarithm of the prior
        log_prior += ss.gamma.logpdf(p[0],1.0,scale=np.sqrt(1.5*(sirs.nu+sirs.mu)))
        log_prior += ss.beta.logpdf(p[1],1.1,1.1)
        log_prior += ss.beta.logpdf(p[2],1.1,1.1)
        log_prior += ss.beta.logpdf(p[3],1.1,1.1)
        log_prior += ss.beta.logpdf(p[4],1.1,1.1)        
        log_prior += ss.beta.logpdf(p[5],1.1,1.1)        
        log_prior += ss.beta.logpdf(p[6],1.1,1.1)                
        log_likelihood = 0.0 #initialize the logarithm of the likelihood
        mu = p[6]*np.diff(sirs.N*sirs.solve(p)[:,-1]) # mean
        omega = 2.0
        theta = 5.0
        ...
        #log_likelihood += np.sum(ss.norm.logpdf(sirs.data[1:],loc=mu,scale=50.0)) # normal        
        print(-log_likelihood - log_prior)
        return -log_likelihood - log_prior
    
""" run the twalk """
syncytial = pytwalk.pytwalk(n=7,U=energy,Supp=support)
syncytial.Run(T=100000,x0=init(),xp0=init())
...
plot_range = [(quantiles[0,x],quantiles[1,x]) for x in np.arange(7)]
labels =  [r"$b_0$", r"$b_1$",r"$I(0)$", r"$R(0)$",r"$x(0)$", r"$y(0)$", r"$q$"]
...
for index in np.arange(100):
    ...
    mu = p[6]*np.diff(sirs.N*sirs.solve(p)[:,-1])
...
ax.plot(sirs.tdata,sirs.data,'ro-')
ax.errorbar(sirs.tdata,solns_median,yerr=solns_quant,fmt='.',ecolor='k',lw=0.5)